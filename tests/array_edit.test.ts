...
describe("array edit", () => {
  ...
  describe("invalid case", () => {
    it("set out of boundary", () => {
      expect(() => edit(array, "foo.bar.[12]", 4, opt)).toThrowErrorMatchingInlineSnapshot(
        `"Key Error: index out of boundary '12' for 'foo.bar'"`,
      )
      expect(() => edit(array, "foo.bar.[4]", 4, opt)).toThrowErrorMatchingInlineSnapshot(
        `"Key Error: index out of boundary '4' for 'foo.bar'"`,
      )
    })

    it("empty index", () => {
      expect(() => edit(array, "foo.bar.[]", 4, opt)).toThrowErrorMatchingInlineSnapshot(
        `"Key Error: invalid key '[]'"`,
      )
    })

    it("negative index", () => {
      expect(() => edit(array, "foo.bar.[-1]", 4, opt)).toThrowErrorMatchingInlineSnapshot(
        `"Key Error: invalid key '[-1]'"`,
      )
    })

    it("range index", () => {
      expect(() => edit(array, "foo.bar.[1:3]", 4, opt)).toThrowErrorMatchingInlineSnapshot(
        `"Key Error: invalid key '[1:3]'"`,
      )
    })
  })
  ...
  describe("array boundary tests", () => {
    it("empty array boundary", () => {
      ...
      expect(() => edit(emptyArray, "foo.bar.[1].baz", 4, opt)).toThrowErrorMatchingInlineSnapshot(
        `"Key Error: index out of boundary '1' for 'foo.bar.[1]'"`,
      )
    })
    ...
    it("large index out of boundary", () => {
      expect(() => edit(array, "foo.bar.[999]", 1, opt)).toThrowErrorMatchingInlineSnapshot(
        `"Key Error: index out of boundary '999' for 'foo.bar'"`,
      )
    })
    ...
    it("boundary in nested array", () => {
      const nested = dedent`
                ...
                bar = [[1,2], [3,4]]
            `
      expect(() => edit(nested, "foo.bar.[3]", [5, 6], opt)).toThrowErrorMatchingInlineSnapshot(
        `"Key Error: index out of boundary '3' for 'foo.bar'"`,
      )
    })
  })
})
